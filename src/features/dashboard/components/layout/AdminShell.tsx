...
export default function AdminShell({ children }: AdminShellProps) {
  ...
  const handleSignOut = () => {
    ...
    // Full navigation prevents request-abort race and ensures server clears session cookie.
    window.location.assign("/api/admin/auth/logout?redirect=/dashboard/login");
  };
      ...
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        <aside className="border-b border-white/10 bg-[#081524] px-5 py-6 lg:border-r lg:border-b-0">
          ...
          <button
            ...
            className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-70"
          >
